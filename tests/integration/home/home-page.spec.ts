...
describe('WP-07.6 Home integration boundaries', () => {
  ...
  it.each([
    ...
    expect(available.map((item) => item.href)).toEqual([`/${locale}`, `/${locale}/map`, `/${locale}/behaviours`])
    expect(unavailable).toHaveLength(4)
    expect(unavailable.every((item) => item.href === undefined)).toBe(true)
    expect(content.entryPoints.filter((item) => item.available).map((item) => item.href)).toEqual([`/${locale}/map`, `/${locale}/behaviours`])
  })
